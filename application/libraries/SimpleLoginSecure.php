...
class SimpleLoginSecure
{
	 ...
	 * @return	bool
	 */
	function create($user_email = '', $user_pass = '', $rol='admin', $auto_login = false) 
	{
		...
		$this->CI->db->where('userEmail', $user_email); 
	...
	*/
	...
	function login($user_email = '', $user_pass = '') 
	{
		...
		if ($query->num_rows() > 0) 
		{
			...
			if(!$hasher->CheckPassword($user_pass, $user_data['userPass']))
				return 'Contraseña inválida';

			if($user_data['acceso']=='no')
				return 'Acceso denegado por el administrador';
			...
			$user_data['user'] = $user_data['userEmail']; // for compatibility with Simplelogin
	...
	function delete($user_id) 
	{
		...
		return $this->CI->db->delete($this->user_table, array('userId' => $user_id));
	}
